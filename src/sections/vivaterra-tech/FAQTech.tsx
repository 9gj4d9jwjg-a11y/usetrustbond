...
const faqs = [
  { question: 'Como acompanho o impacto da minha contribuição?', answer: 'Você terá acesso ao dashboard exclusivo com métricas em tempo real, evidências blockchain e atualizações diretas da VivaTerra.' },
  { question: 'E se o projeto não atingir a meta?', answer: 'Seu investimento é reembolsado integralmente via Trustbond, garantindo zero risco para você.' },
  { question: 'Como a transparência é garantida?', answer: 'Todo o processo usa blockchain para registros imutáveis, com verificação IA e humana para marcos chave.' },
];

export default function FAQTech({ className = '' }: { className?: string }) {
  ...
  return (
    <section ref={sectionRef} className={`relative py-24 lg:py-32 bg-[#14331A] ${className}`}>
      <div className="max-w-3xl mx-auto px-6 sm:px-8">
        ...
        <div ref={faqsRef} className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className={`rounded-2xl border transition-all duration-300 ${openIndex === i ? 'bg-[#0A1F0A]/30 border-[#4ADE80]/30' : 'bg-white/5 border-white/10 hover:border-white/20'}`}>
