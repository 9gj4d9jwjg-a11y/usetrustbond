...
const metrics = [
  { icon: Users, value: 20, suffix: 'M', label: 'Pessoas alcançadas (alvo)', description: 'Grande São Paulo' },
  { icon: Package, value: 12, suffix: '', label: 'Novos SKUs de produtos', description: 'Linha eco-limpeza completa' },
  { icon: MapPin, value: 3200, suffix: '+', label: 'Pontos de coleta', description: 'Em toda a região' },
  { icon: DollarSign, value: 20, suffix: 'k', label: 'Receita atual/mês', description: 'A ser estabilizada' },
];
...
function AnimatedNumber({ value, suffix }: { value: number; suffix: string }) {
  ...
  useEffect(() => {
    const trigger = ScrollTrigger.create({ trigger: numberRef.current, start: 'top 85%',
      onEnter: () => { gsap.to({ val: 0 }, { val: value, duration: 2, ease: 'power2.out', onUpdate: function () { setDisplayValue(Math.floor(this.targets()[0].val)); } }); },
      once: true });
    return () => trigger.kill();
  }, [value]);
...
export default function ImpactMetricsEco({ className = '' }: { className?: string }) {
        ...
        <div ref={cardsRef} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {metrics.map((m, i) => (
            <div key={i} className="group relative p-8 rounded-3xl bg-[#0A1F0A]/50 border border-white/10 hover:border-[#4ADE80]/40 transition-all duration-500 hover:-translate-y-2 text-center">
              ...
              <div className="text-4xl sm:text-5xl font-bold text-[#4ADE80] mb-2"><AnimatedNumber value={m.value} suffix={m.suffix} /></div>
